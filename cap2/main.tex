...
\item PNG (\emph{Portable Network Graphics}) ou JPG (\emph{Joint Photographic Experts Group}) para imagens em bitmap, como fotografias;

\item PDF (\emph{Portable Document Format}) ou EPS (\emph{Extended PostScript}) para desenhos vetoriais como diagramas e gráficos\footnote{NUNCA use JPG ou GIF para desenhos vetoriais, pois o resultado final geralmente fica borrado.}.

\end{itemize}

A maior parte das ferramentas permite exportar figuras nesses formatos (a figura do exemplo foi produzida com o \emph{Inkscape}, um programa livre para Windows e Linux). A figura \ref{comun-intra-inter} mostra um exemplo de inclusão de figura em PDF.

% exemplo de inserção de figura
\begin{figure}[!htb]
\centering
\includegraphics[width=12cm]{cap2/exemplo-figura.pdf}
...
Equações destacadas devem ser numeradas como segue:
...
\lstinputlisting{cap2/exemplo.c}
